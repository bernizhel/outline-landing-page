...
function appearAnimation() {
    ...
    if (
        !mainAnimated &&
        window.scrollY >= mainElem.offsetTop - offsetAnimation
    ) {
    ...
    if (
        !clientsAnimated &&
        window.scrollY >= clientsElem.offsetTop - offsetAnimation
    ) {
    ...
    if (
        !featuresAnimated &&
        window.scrollY >= featuresElem.offsetTop - offsetAnimation
    ) {
    ...
    if (
        !designAnimated &&
        window.scrollY >= designElem.offsetTop - offsetAnimation
    ) {
    ...
    if (
        !repliesAnimated &&
        window.scrollY >= repliesElem.offsetTop - offsetAnimation
    ) {
    ...
    if (
        !statisticsAnimated &&
        window.scrollY >= statisticsElem.offsetTop - offsetAnimation
    ) {
    ...
    if (
        !questionsAnimated &&
        window.scrollY >= questionsElem.offsetTop - offsetAnimation
    ) {
...
function goToSection() {
    ...
    window.scrollTo({
        top:
            endpoint.offsetTop -
            (asideHeader.offsetHeight || headerElem.offsetHeight - 40),
        left: 0,
        behavior: 'smooth',
    });
}
...
// Replies
// Replies
const contentContainerElem = document.querySelector(
    '.replies .content_container',
);
...
function switchReplies(
    toRight = true,
    initialPosX = 0,
    animFunc = 'cubic-bezier(0.39, 0.58, 0.5, 1)',
) {
    ...
    contentContainerElem.insertBefore(
        tempReply,
        contentContainerElem.children[toRight ? 3 : 1],
    );
...
window.addEventListener('scroll', () => {
    if (
        window.scrollY >=
            contentContainerElem.parentElement.offsetTop - offsetAnimation &&
        !switched
    ) {
...
function circleMenuActivation() {
    ...
    circleMenuElem
        .querySelector(
            '#circle' +
                document.querySelector('.replies .item.active').id.slice(5),
        )
...
function circleMenuSwitch() {
    ...
    let idDifference =
        +document.querySelector('.replies .item.active').id.slice(5) - idNumber;
...
for (let circleContainerElem of circleMenuElem.children) {
    circleContainerElem.addEventListener(
        'click',
...
arrowLeftElem.addEventListener(
    'click',
    suspendIntervalSwitching(switchReplies, false),
);
arrowRightElem.addEventListener(
    'click',
    suspendIntervalSwitching(switchReplies),
);
...
function statsShow() {
    if (
        scrollY >
            statisticsElems[0].parentElement.offsetTop - offsetAnimation &&
        !statsShown
    ) {
        statisticsElems.forEach((item, i) => {
            animatedCounter.call(
                item,
                Number(item.firstElementChild.innerHTML),
            );
        });
        statsShown = true;
    }
